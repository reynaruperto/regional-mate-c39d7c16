-- Fix the create_notification_from_like trigger to use correct column name
...
BEGIN
  ...
  IF NEW.liker_type = 'whv' AND NEW.liked_job_post_id IS NOT NULL THEN
    ...
    -- Check if employer has notifications enabled (FIXED: notifications_enabled not notification_enabled)
  ...
  ELSIF NEW.liker_type = 'employer' AND NEW.liked_whv_id IS NOT NULL THEN
    ...
    -- Check if WHV has notifications enabled (FIXED: notifications_enabled not notification_enabled)
    SELECT notifications_enabled INTO v_notification_enabled

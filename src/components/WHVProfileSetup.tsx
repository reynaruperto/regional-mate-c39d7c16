...
const WHVProfileSetup: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (cError) console.error("Country fetch error:", cError);
      if (sError) console.error("Visa stage fetch error:", sError);
  ...
  // ✅ Load eligible visa stages for the selected country
  useEffect(() => {
    ...
    const fetchEligibility = async () => {
      ...
      if (error) {
        ...
        setEligibleStages(data?.map((d) => d.stage_id) || []);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Pass country + visa to next step

...
interface VisaStage { stage_id: number; label: string; sub_class: string; stage: number | null; }
...
const WHVEditProfile: React.FC = () => {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      ...
      // industries + roles (eligible)
      const { data: eligibleIndustries } = await supabase.from("temp_eligibility").select("industry_id, industry_name");
  ...
  const handleSave = async () => {
    ...
    if (step === 1) {
      ...
      await supabase.from("maker_visa").update({ expiry_date: visaExpiry }).eq("user_id", user.id);
    }

    if (step === 2) {
      await supabase.from("whv_maker").update({ tagline }).eq("user_id", user.id);
      // TODO: clear + reinsert maker_preference for new industries/roles/regions
    }
    ...
    if (step === 3) {
      ...
      if (otherLicense) {
        ...
        if (other) {
          await supabase.from("maker_license").upsert({ user_id: user.id, license_id: other.id, other: otherLicense });
        }
      }
      ...
      <div className="w-[430px] h-[932px] bg-black rounded-[60px] p-2 shadow-2xl">
        <div className="w-full h-full bg-white rounded-[48px] flex flex-col">
          ...
          <div className="px-4 py-3 border-b flex justify-between items-center">
            <button onClick={() => navigate("/whv/dashboard")} className="text-orange-500 underline">Cancel</button>
            <h1 className="text-lg font-medium">Edit Profile</h1>
            <button onClick={handleSave} className="flex items-center text-orange-500 underline"><Check size={16} className="mr-1" /> Save</button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-6 space-y-6">
            ...
            {step === 2 && (
              <>
                ...
                <Textarea value={tagline} onChange={(e) => setTagline(e.target.value)} placeholder="Tagline" />
                <h3>Industries</h3>
                {industries.map((i) => (
                ...
                {roles.filter(r => selectedIndustries.includes(r.industryId)).map((r) => (
                  <button key={r.id} onClick={() => setSelectedRoles(selectedRoles.includes(r.id) ? selectedRoles.filter(x => x !== r.id) : [...selectedRoles, r.id])}
            ...
            {step === 3 && (
              <>
                ...
                <Button onClick={() => setWorkExperiences([...workExperiences, { id: Date.now().toString(), industryId: null, position: "", company: "", location: "", startDate: "", endDate: "", description: "" }])}>Add</Button>
                {workExperiences.map((exp) => (
                  <div key={exp.id} className="border p-2">
                    ...
                    <Textarea value={exp.description} onChange={(e) => setWorkExperiences(workExperiences.map(w => w.id === exp.id ? { ...w, description: e.target.value } : w))} />
                    <Button onClick={() => setWorkExperiences(workExperiences.filter(w => w.id !== exp.id))}><X size={16} /></Button>
                  </div>
                ))}
                <h2>Licenses</h2>
                {allLicenses.map((l) => (
                ...
                <Button onClick={() => setJobReferences([...jobReferences, { id: Date.now().toString(), name: "", businessName: "", email: "", phone: "", role: "" }])}>Add</Button>
                {jobReferences.map((ref) => (
                  <div key={ref.id} className="border p-2">
                    ...
                    <Button onClick={() => setJobReferences(jobReferences.filter(r => r.id !== ref.id))}><X size={16} /></Button>
                  </div>
                ))}
              </>
            )}
          ...
          <div className="p-4 flex flex-col items-center">
            <div className="flex gap-2 mb-3">
              {[1, 2, 3].map((i) => <div key={i} className={`h-2 w-6 rounded-full ${step === i ? "bg-orange-500" : "bg-gray-300"}`} />)}
            </div>

// src/pages/whv/WHVEmployerFullProfileCard.tsx
...
const WHVEmployerFullProfileCard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchJobAndEmployer = async () => {
      try {
        ...
        console.error("Error fetching employer full profile:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (!jobDetails || !employer) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p className="text-gray-600">Employer not found</p>
        <Button onClick={() => navigate("/whv/matches")} className="mt-4">
          Back to Matches
        </Button>
      </div>
    );
        ...
        <div className="w-full h-full bg-white rounded-[48px] overflow-hidden relative">
          ...
          <div className="w-full h-full flex flex-col relative bg-gray-50">
            ...
            <div className="px-6 pt-16 pb-4 bg-white shadow-sm flex items-center justify-between">
              ...
              <h1 className="text-lg font-semibold text-gray-900">Employer Profile</h1>
                ...
                <div className="flex flex-col items-center">
                  ...
                  {employer.mobile_num && (
                    <p className="text-sm text-gray-700 flex items-center mt-1">
                      <Phone size={14} className="mr-1 text-[#1E293B]" /> {employer.mobile_num}
                    </p>
                  )}
                  {employer.email && (
                    <p className="text-sm text-gray-700 flex items-center mt-1">
                      <Mail size={14} className="mr-1 text-[#1E293B]" /> {employer.email}
                    </p>
                  )}
                  <p className="text-sm text-gray-700 flex items-center mt-1">
                    <Globe size={14} className="mr-1 text-[#1E293B]" /> {employer.website}
                  </p>
                </div>

...
const WHVWorkPreferences: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      // Get region rules (state + area + postcode_range)
      const { data: regionData } = await supabase
        .from("region_rules")
        .select("state, area, postcode_range, industry_id")
        .eq("stage", visa.stage_id)
        .eq("sub_class", visaStage?.label.includes("417") ? "417" : "462");
  ...
  const getAvailableAreasForState = (state: string) => {
    return regions.filter((r) => r.state === state).map((r) => ({
      area: r.area,
      note: r.postcode_range && r.postcode_range !== "All postcodes" ? r.postcode_range : null,
    }));
  };
                ...
                <div className="px-4 pb-4 border-t space-y-4">
                  ...
                  {preferredStates.length > 0 && (
                    <div>
                      ...
                      {preferredStates.flatMap((s) => getAvailableAreasForState(s)).map(({ area, note }) => (
                        <label key={area} className="flex items-center justify-between py-1">

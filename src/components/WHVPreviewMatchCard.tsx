    ...
    const fetchProfileData = async () => {
      try {
        ...
        if (visa?.visa_stage) {
          ...
          visaType = label.includes(sub_class)
            ? label
            : `${sub_class} (${label})`;
        }
        ...
        const formattedWorkPrefs: Preference[] = (workPrefsData || []).map(
          (pref: any) => ({
        ...
        const formattedLocationPrefs: Preference[] = (locationPrefsData || []).map(
          (loc: any) => ({
            state: loc.state,
        ...
        const formattedExperiences: WorkExperience[] = (experiences || []).map(
          (exp: any) => {
            const industry = industryData?.find(
              (ind: any) => ind.industry_id === exp.industry_id
            );
            return {
            ...
            <div className="border-2 border-orange-500 rounded-2xl p-6 space-y-6">
              ...
              {locationPreferences.length > 0 && (
                <div>
                  ...
                  {locationPreferences.map((pref, i) => (
                    <p key={i} className="text-sm text-gray-700">
                      {pref.area}, {pref.state}
                    </p>
                  ))}
                </div>
              )}
